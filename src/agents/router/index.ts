...
const AGENT_VOCABS: AgentVocab[] = [
  ...
  },
  {
    ...
    keywords: [
      ...
      'trip', 'itinerary', 'agenda', 'timeline', 'milestones',
      'خطة', 'تخطيط', 'استراتيجية', 'مشروع', 'خطوات', 'مراحل', 'نظم', 'نظّم',
      'إدارة', 'جدول', 'سير عمل', 'رحلة', 'برنامج رحلة',
    ],
    patterns: [
      /\b(plan|outline|strategize)\s+(how|for|to|a|an|the)?\b/i,
      /\b(help me|let's)\s+(plan|organize|coordinate|manage)\b/i,
      /\b(step[\s-]?by[\s-]?step|multi[\s-]?step)\b/i,
      /\b(plan|plan a|plan an)\s+\d+[\s-]?(day|week|hour|month)\b/i,
      /\b\d+[\s-]?day\s+(trip|itinerary|plan|schedule)\b/i,
      /\b(خطة|تخطيط|خطوات|خطة رحلة)\b/i,
    ],
  },
];
